...
## Permanent Immigrants Inflows from 2009 to 2019
...
merged_data %>%
  group_by(Year) %>%
  summarize(Permanent_Inflows = sum(Permanent_Inflows, na.rm = TRUE)) %>%
  ggplot(aes(x = Year, y = Permanent_Inflows)) +
  geom_line(group = 1, color = "red") +
  scale_x_continuous(breaks = seq(2009, 2019, by = 1), labels = c("2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019")) +
  theme_bw() +
  labs(
    title = "Permanent Immigrants Inflows in Canada from 2009 to 2019",
    x = "Year",
    y = "Permanent Immigrants Inflows" 
  )
...
merged_data %>%
  group_by(Year) %>%
  summarize(Employment_Rate = sum(Employment_Rate, na.rm = TRUE)) %>%
