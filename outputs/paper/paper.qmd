...
merged_data |>
  mutate(Permanent_Inflows_Rate = (Permanent_Inflows / Population) * 100) |>
  group_by(Year) |>
  summarize(Permanent_Inflows_Rate = sum(Permanent_Inflows_Rate, na.rm = TRUE)) |>
  ggplot(aes(x = Year, y = Permanent_Inflows_Rate)) +
...
merged_data <-
  merged_data |>
  mutate(Permanent_Inflows_Rate = (Permanent_Inflows / Population) * 100) |>
  mutate(Centred_Employment_Rate = Employment_Rate - mean(merged_data$Employment_Rate))

merged_data_centered_model <-
  lm(
    Permanent_Inflows_Rate ~ Employment_Rate,
    data = merged_data
  )
...
merged_data <- merged_data %>%
  ...
  ggplot(merged_data, aes(x = Employment_Rate, y = Permanent_Inflows_Rate)) +
  ...
  labs(
    x = "Employment Rate (percentage)",
    y = "Permanent Inflows Rate (percentage)"
  ) +
  theme_classic()
